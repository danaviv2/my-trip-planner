...
החזר JSON בלבד (ללא markdown):
{
  ...
  const generateExpenseSummary = async (tripExpenses) => {
    ...
    // ── הוסר "של טיול לישראל" ──
    // היעד היה קשיח בפרומפט, בזמן ש-`tripName` מוזרק שלוש שורות
    // מתחת. המודל קיבל שתי עובדות סותרות — "ישראל" מול "רומא" —
    // והסיכום יכול היה לדבר על מחירים בישראל בטיול לאיטליה.
    // דפוס 2: ערך סביר שנלקח מהמקום הלא נכון.
    // אין כאן שכבת מטמון (אומת: אפס אזכורים בפונקציה), ולכן השינוי
    // אינו דורש העלאת גרסת מטמון כמו בשירותי ה-AI האחרים.
    const prompt = `אתה יועץ פיננסי לטיולים. סכם את ההוצאות הבאות בעברית קצרה, ידידותית ועם אמוג'י רלוונטיים. ציין: מה ההוצאה הגדולה, מה הממוצע ליום, האם הוציא הרבה/מעט, טיפ חיסכון אחד.
