  ...
  return (
    <Box sx={{
      ...
      <Box sx={{
        ...
        // מקצה לקצה. זה מה שגרם למשתמש לקרוא לה "ענקית", ולא ההדגמה
        // שבתוכה. תקרת רוחב ופינות מעוגלות הופכות אותה לכרטיס שיושב
        // בדף במקום לבלוע אותו, וגם נותנות למלבן הפנימי יחס סביר.
        maxWidth: 1100,
        mx: 'auto',
        ...
        py: hasUpcoming ? { xs: 2, md: 2.5 } : { xs: 2, md: 2.5 },
        px: { xs: 2, md: 3 },
        textAlign: 'center',
        position: 'relative',
        overflow: 'hidden'
        ...
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
          <Typography variant="h2" component="h1" sx={{
            fontWeight: 800,
            // ── הגופן הוקטן ב-07.09.2026, יחד עם הכותרת הארוכה ──
            // הנוסח החדש הוא 65 תווים מול 34 קודם. ב-3.5rem הוא נמדד
            // 146px בשתי שורות, ודחף את ה-Hero אל **מעבר** לקיפול:
            // הבלוק הבא הציץ ‎-12px, כלומר בכלל לא. מסך שממלא בדיוק את
            // הגובה מסתיר את עצם קיומו של שאר הדף — מלכודת שהפרויקט
            // כבר שילם עליה כשההצצה הייתה 22px.
            fontSize: hasUpcoming
              ? { xs: '1.15rem', sm: '1.4rem', md: '1.6rem' }
              : { xs: '1.45rem', sm: '1.9rem', md: '2.4rem' },
            mb: hasUpcoming ? 0.5 : 1.5,
          ...
          {/* ── תת-הכותרת חזרה ב-07.09.2026, בהחלטת המשתמש ──
              היא הוסרה ב-04.09 מסיבה מתועדת: הנוסח הישן ("מסלולים
              מותאמים אישית • טיפים מקומיים • מידע מקיף") היה הבטחה
              גנרית שכל מתכנן טיולים יכול לכתוב, והוא ישב בדיוק בין
              הכותרת להדגמה — כלומר דחף מטה את הדבר היחיד בדף שמוכיח
              במקום להבטיח.

              הנוסח החדש אינו גנרי: הוא נוקב במה שהמשתמש עושה היום
              (אקסלים, המלצות מאינסטגרם) ובמה שיוצא מזה. ההתנגדות
              המקורית לא חלה עליו.
          ...
          {/* ── ההדגמה יושבת כאן, ולא מתחת ל-Hero ──
              הגרסה הקודמת הייתה מכווצת ומתחת לסגול, ולא נתפסה בעין:
              המשתמש דיווח שגם מי שמכיר את האפליקציה איתר אותה רק אחרי
              כמה שניות. אין טעם להתחרות ב-Hero — הכרטיסים הלבנים על
              הסגול הם הניגוד שעוצר את העין, וזה מה שהיא צריכה.

              מוצגת רק לאורח: מי שיש לו נסיעה ראה את הדבר האמיתי בכרטיס
