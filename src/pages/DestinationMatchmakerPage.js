  ...
  const handleAnswer = async (label) => {
    ...
    if (currentQ < 4) {
      ...
      try {
        ...
        const q5 = label;
        const prompt = `אתה יועץ טיולים מומחה שמייעץ למטיילים היוצאים מישראל (נמל התעופה בן גוריון).
...
כללי חובה — יעד שלא עומד בהם אינו קביל:

1. זמן טיסה מישראל מול משך הטיול. חשב את זמן הטיסה בפועל מתל אביב.
   - "סוף שבוע (2-3 ימים)": רק יעדים עד כ-4 שעות טיסה. אירופה קרובה בלבד
     (קפריסין, יוון, איטליה, בלקן). אסור לחלוטין: אסיה, אמריקה, אפריקה
     שמדרום לסהרה, וכל יעד עם קונקשן.
   - "שבוע": עד כ-6 שעות טיסה.
   - "שבועיים" ומעלה: אין הגבלה.

2. העונה שנבחרה. השדה bestTime חייב לכלול את העונה ${q5} או חודשים
   שנופלים בתוכה. אין להמליץ על יעד שהעונה הטובה בו היא עונה אחרת.

3. היקף מתאים למשך. ל"סוף שבוע" המלץ על עיר אחת או אזור קומפקטי,
   לא על מסלול בין מספר ערים. אין להציע 3 ערים לטיול של 2-3 ימים.

4. הרכב הנוסעים. אם נבחר "${q3}" — כל יעד חייב לכלול לפחות היילייט אחד
   שמתאים ספציפית להרכב הזה.

5. matchScore חייב לשקף עמידה בכללים 1-3. יעד שעומד בכולם: 90+.
   יעד שמתפשר על אחד מהם: מתחת ל-80.
